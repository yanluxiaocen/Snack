...
## 一句话简介
控制台贪吃蛇（C++ / STL，**CMake 双机构建**，简历向项目）。游戏规则已闭环 + 手感优化完成；**工程化四件套齐活**（目录拍平 / CMake / doctest 单测 / README）；**BFS 自动寻路 AI 全部上线**：追食物 → 单步安全评估 → **路径级前瞻（虚拟推演整条路径）** + **饥饿兜底（破活锁）**；17 用例 / 22 断言全绿、编译零警告。图形版（SFML）决策待办。
...
### 待办（下次严格按此顺序）
- [ ] **确认 598791e 已推送**（换机前）——本机校园网连不上 GitHub，开 Steam++ 或到主力机 `git push`
- [ ] **实机验收 B 的效果**：按 E 托管观察——绕圈是否明显减少、分数是否持续增长、是否偶尔送死（**偶尔正常**；若经常送死说明路径级判定偏松，需收紧）
- [ ] **主力机交接**：pull 后别开 .sln（已退役），VS"打开文件夹"→ CMake（MSVC kit）；跑单测确认 17 用例 / 22 断言全绿 + `/W4` 零警告
- [ ] **待决策：图形版（SFML）还是结项换项目**：
  - 路线①：**1 小时低成本试水**——主力机只验证"SFML 空窗口能编能跑"，不通过就停（项目已完整，零损失）
  - 路线②：直接结项，转下一个项目
  - 关键风险：**SFML 官方无 GCC 16 预编译包**，本机 MinGW ABI 风险高 → 若做，走主力机 MSVC + 官方 VS2022 二进制最稳
- [ ] 可选小项：**D 方案（追尾挑最优方向：在能摸到尾巴的走法里选"可达空地最大"的那步，把绕圈变成有方向的自救）**；给路径级判定补回归测试（构造"单步不安全但整条路径安全"的局面）；**统计面板**（最大长度 / 存活步数，让 A/B 效果可量化）；Hamiltonian 回路；GitHub Actions CI；rand 换 `<random>`；draw 显示当前模式（手动/AI）；真截图替换 ASCII 示意图
