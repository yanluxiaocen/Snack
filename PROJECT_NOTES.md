...
## 一句话简介
控制台贪吃蛇（C++ / STL，**CMake 双机构建**，简历向项目）。游戏规则已闭环 + 手感优化完成；**工程化四件套齐活**（目录拍平 / CMake / doctest 单测 / README）；**BFS 自动寻路 AI 全部上线**：追食物 → 单步安全评估 → **路径级前瞻（虚拟推演整条路径）** + **饥饿兜底（破活锁）**；17 用例 / 22 断言全绿、编译零警告。**AI 章节已结项**（验收数据：典型单局得分 70~80 → 120 → **170**）；项目处于"随时可展示"状态——图形版（SFML）与 CI 留作后续可选。
...
- [x] **仓库迁移 Save → Memory（2026-09-16）**：整个工作区目录改名，Snack 现在位于 `D:\Tool\VSCode\Memory\Snack`；踩坑：build/CMakeCache.txt 写死的旧绝对路径失效 → 删 build 重配（见踩坑 #27）
- [x] **AI 阶段 2：BFS 单测（2026-09-16）**：新建 tests/test_autopilot.cpp（6 用例：直线追食物 / 向右 / 长度 1 可反向 / 绕开自身 L 形障碍 / 尾巴不算障碍 / 食物不可达保底）；unit_tests target 加入 AutoPilot.cpp；**17 用例 / 22 断言全绿**。过程中测试抓到两个隐蔽 bug（返回值被丢弃、逗号表达式）——见踩坑 #28/#29
- [x] **AI 阶段 3 前两步（2026-09-16）**：① **重构抽工具**（`bfsFirstStep` / `buildBlocked` / `stepToDirection`，全部 static 文件私有）——重构后 17 条测试仍全绿（行为未变）；② **`isMoveSafe` 实现**（模拟走一步 → 用走完后的障碍表检查新头到新尾巴是否可达）
- [x] **开启编译警告（2026-09-16）**：两个 target 统一 `-Wall -Wextra`（MSVC 侧 `/W4`，抽成 SNACK_WARN_FLAGS 变量复用）；开启当天就抓到 **UI.cpp `score` 未使用 = 分数显示回归**（改光标重绘时漏了"分数："那行，已补回）——见踩坑 #31
- [x] **AI 阶段 3 接线（2026-09-17）**：`nextDirection` 改为三层决策（安全食物 → 追尾保命 → 维持方向）；`isMoveSafe` 正式上岗，**`-Wunused-function` 警告消失＝接线成功的铁证**；17/22 全绿
- [x] **实机发现"活锁"（2026-09-17）**：蛇有时变成**衔尾蛇**——永远绕圈不吃食物（哪怕食物明显可达）。根因：安全判定**只模拟一步**，盘圈时"朝食物的第一步"常被判不安全 → 永远落到追尾分支。**这不是 bug 而是启发式视野不足（局部最优）**
- [x] **A 方案：饥饿兜底（2026-09-17）**：`nextDirection` 新增 `stepsSinceEat` 参数（**带默认值 0**，6 条老测试零改动）；连续 `HUNGER_LIMIT=80` 步没吃到食物 → 放宽安全要求"赌一把"。状态归 main 持有（不用 static 局部变量 → AI 保持纯函数、单测不受污染）。实机：活锁被破除 ✅
- [x] **B 方案：路径级安全判定（2026-09-17）**：新增 `bfsPath`（返回**整条路径**）+ `simulateWalk`（沿路径虚拟推演，含吃到食物那步变长）+ `isPathSafe`（走完整条路后判尾巴可达性）；`bfsFirstStep` 改为复用 `bfsPath`（去重）；**`isMoveSafe` 退役删除**。效果：绕圈明显减少、吃食物更主动；零警告 + 17/22 全绿
- [x] **AI 结项 + 文档定稿（2026-09-20）**：验收数据——**典型单局得分 70~80（纯 BFS）→ 120（单步安全）→ 170（路径级 + 饥饿兜底）**，蛇长 171 节约占场地 1/3；据此判定**到此为止**（剩余提升空间 D 方案 / Hamiltonian 写入 README"后续计划"，不再实现）；README 新增 **"🤖 AI 迭代历程"** 表格（三轮迭代 + 症状 + 得分对照），"后续计划"同步更新。项目进入"随时可展示"状态

### 待办（下次严格按此顺序）
- [ ] **提交推送**（README + 日志本轮改动）
- [ ] **主力机交接**：pull 后别开 .sln（已退役），VS"打开文件夹"→ CMake（MSVC kit）；跑单测确认 17 用例 / 22 断言全绿 + `/W4` 零警告
- [ ] **下一个项目**：已与 AI 讨论候选方向（见"关键决定"），待用户拍板
- [ ] 后续可选（**不着急，有空再做**）：GitHub Actions CI（push 自动构建 + ctest + README 徽章）；SFML 图形版试水（主力机 1 小时，先只验空窗口）；真截图替换 ASCII 示意图；`rand` 换 `<random>`；`draw` 显示当前模式（手动/AI）
- [ ] 若日后想继续卷 AI（当前判定为收益递减）：D 方案（追尾挑"可达空间最大"方向）；Hamiltonian 回路
...
- **AI 安全性判据（2026-09-16）**：**能摸到自己的尾巴 = 还能活**——身体是堵会动的墙，而**尾巴是唯一会让出格子的部分**；只要头到尾巴有通路，蛇就能"跟着尾巴流动"永不被困死。安全是"**走完这步之后**"的属性，所以必须**虚拟走一步**再评估；**变长时尾巴不动**，而吃食物那一步恰恰最危险（多占一格可能封死最后退路）
- **AI 决策三层优先级（2026-09-16）**：① 食物可达且安全 → 去吃；② 否则 BFS 追自己的尾巴（保命，会看到它"放着食物不吃去绕圈"，属正确行为）；③ 都不行 → 维持当前方向
- **编译警告开满**：两 target 统一 `-Wall -Wextra`（MSVC `/W4`），抽成 `SNACK_WARN_FLAGS` 变量复用 —— 实测当天就抓到真 bug
- **饥饿兜底的状态归属（2026-09-17）**：饥饿计数器放 **main**（游戏循环持有）、用**参数**传给 AI，**绝不用 static 局部变量**——后者会破坏纯函数、且让单元测试**跨用例共享状态**（用例顺序影响结果）；签名加**默认参数** `stepsSinceEat = 0` 保持老调用点零改动
- **安全判据从"单步"升级为"路径级"（2026-09-17）**：单步模拟会误判（第一步看着危险、走完整条路其实安全）→ 活锁；改为**沿整条路径虚拟推演**（含吃完那步变长）再判尾巴可达性，`isMoveSafe` 退役
- **AI 四层决策定型**：① 路径级安全 → 去吃；② 否则追尾保命；③ 否则维持方向；④ **饥饿兜底**：80 步没吃到就放宽 ①（赌一把破活锁）
- **AI 停止标准（2026-09-20）**：以**"可量化收益递减"**为停止依据——三轮迭代把典型单局得分从 70~80 推到 170 后，剩余改进（D 方案 / Hamiltonian）投入产出比明显下降 → 判定结项；未实现的想法**写进 README"后续计划"而不是继续堆功能**。原则：**项目要"完成"而不是"完美"**
...
27. **目录改名/移动后 build/ 里的 CMakeCache.txt 失效**（里面写死了源码绝对路径）→ 报 `CMakeCache.txt directory ... is different than the directory ... where it was created`；**注意构建失败后旧 exe 还在，直接跑测试会"假通过"** → 删掉 build/ 重新配置
28. **有返回值的函数调用结果被丢弃**：`stepToDirection(...);` 忘了 `return` → 函数永远走保底分支（AI 退化成"只会直走"）；当时 6 条测试只挂了 3 条——**期望值碰巧等于默认状态的用例会"蒙对"**，所以写测试要尽量让期望值 ≠ 默认值
29. **逗号表达式不是函数调用**：`return (head, step, snake.getDirection());` 完全合法，但只是"依次求值取最后一个"（不小心丢了函数名就变成它），编译器一声不吭
30. 函数声明了返回类型却**漏写 return** → `-Wreturn-type` 警告 + 未定义行为（本次 stepToDirection）
31. **`-Wall -Wextra` 的价值**：开启当天就抓到两类真问题——未使用参数（UI.cpp 的 `score` = 分数显示回归）和未使用函数（`isMoveSafe` 定义了却没接线）；**警告不说话 ≠ 没问题，一说话往往是真漏**
32. **抽象提取时，循环的终止条件最容易搬错**：`while (!(parent[cur] == start))` 是**"只取第一步"**的写法；要收**整条路径**必须用 `while (!(cur == start))`。搬错后果：path 缺第一步（**食物相邻时 path 为空 → `path.front()` 越界 = 未定义行为**）
33. **"蒙对"现象（第三次出现）**：期望值恰好等于**当前方向**的用例，会在函数退化（走 fallback 分支）时依然通过 → 写测试要让期望值 ≠ 默认状态；另外 **3 条失败全部返回同一个值（0/Up）= "走了兜底分支"的强信号**，可据此快速定位
34. **启发式的"视野"问题**：单步模拟会活锁（局部最优：永远拒绝、永远绕圈）→ 解法是 **n 步前瞻**（沿整条路径虚拟推演）或加**兜底机制**（如饥饿计数器）
...
**算法（2026-09-10 补充，与 PAT 直接相关）**：网格 BFS（`queue` + `visited` + **父节点表回溯路径**）；障碍建模（蛇身除尾）；`DX/DY` 方向数组与枚举数值对齐技巧；**贪心最短路的局限**——只看当下最优解会把自己困死，需要"可达性检查/洪水填充"这类安全性评估
**AI 与重构（2026-09-16 补充）**：**安全性评估**＝模拟走一步后做**可达性判定**（新头 → 新尾巴能否连通）；"模拟行走"要自己拼出"走完后的身体"（`deque` 的 `push_front` + 按 `keep` 复制，`keep = willGrow ? size : size-1`）；**重构手法**：把内联 BFS 抽成 `static` 文件私有工具（`bfsFirstStep` / `buildBlocked` / `stepToDirection`），**抽完必须跑测试确认行为未变**；编译器警告 `-Wall -Wextra` 能让"死代码/未使用参数"自己冒出来
**AI 进阶（2026-09-17 补充）**：**单步启发式 → n 步前瞻**（沿整条路径虚拟推演后再判定，治活锁/局部最优——"贪心为什么不一定最优"的实战版）；**兜底机制**（饥饿计数器：长期无进展就放宽策略，避免僵死）；**状态归属原则**（可变状态留游戏循环、AI 保持纯函数，默认参数兼容老接口）；启发式的正确性靠**测试 + 实机观测**双重验证
